...
func TestSearch(t *testing.T) {
	...
	collName, _ := createCollectionWithDataIndex(ctx, t, mc, false, false, true)
...
func TestSearchPartitionNotExist(t *testing.T) {
	...
	for _, partitions := range partitionsNotExist {
		...
		common.CheckErr(t, errSearch, false, "has not been loaded to memory or load failed")
	}
}
...
func TestSearchEmptyOutputFields(t *testing.T) {
	...
	for _, enableDynamic := range []bool{true, false} {
		// create collection with data
		collName, _ := createCollectionWithDataIndex(ctx, t, mc, false, enableDynamic, true)
...
func TestSearchTopKHnsw(t *testing.T) {
	...
	common.CheckErr(t, errSearchEmpty, false, "Param 'ef'(7) is not in range [10,")
}
